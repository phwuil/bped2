...
class Pedigree:
  ...
  def pedigree_overview_file(self, filename):
    ...
    f.write(
      f"Dans le pedigree, les peoples suivants apparaissent en tant que mère et père dans le pedigree : {self.check_mother_and_father()}\n")
    f.write(
      f"Dans le pedigree, les peoples suivants ont des liens antérieurs de consanguinité : {self.check_consanguinity_pedigree()}\n")
  ...
  #    """
  #    graph = pydot.Graph(name='Pedigree', strict=True, comment=name)
  #    graph.attr(label=r'\n\n' + name + ' pedigree')
  #    for k, v in self._pedigree.items():
  #        if v.fatID != self.no_people:
  ...
  def graph_pydot(self, name, length):
    ...
    Box = leaves
    Diamond = Nodes
    Color Cyan #00ffff = Male
    Color Pink #ff009c = Female
    Color white = MaleFemale
    Color Dark #000000 = Unknown
    """
    col_rac_txt = {self.sex_undefined: "#000000",
                   self.sex_male: "#00ffff",
                   self.sex_female: "#ff009c",
                   self.sex_malefemale: "white"}
    col_rac_fill= {self.sex_undefined: "#000000",
                   self.sex_male: "#00ffff",
                   self.sex_female: "#ff009c",
                   self.sex_malefemale: "white"}
    ...
    for k, v in self._pedigree.items():
      if v.pID in roots:
        #graph.add_node(pydot.Node(k, shape='doublecircle', fontsize=length, style="filled", color=col_rac_txt[v.sex],
        #                          fillcolor=col_rac_fill[v.sex]))

        # if v.nbrChild() != 0 and v.fatID ==self.no_people and v.matID == self.no_people:
        if v.sex == 1:
          graph.add_node(pydot.Node(k, shape='doublecircle', fontsize=length, style="filled", color="#00ffff",
                                    fillcolor="#227ef0"))
        elif v.sex == 2:
          graph.add_node(pydot.Node(k, shape='doublecircle', fontsize=length, style="filled", color="#ff009c",
                                    fillcolor="#227ef0"))
        elif v.sex == 3:
          graph.add_node(pydot.Node(k, shape='doublecircle', fontsize=length, style="filled", color="white",
                                    fillcolor="#227ef0"))
        else:
          graph.add_node(pydot.Node(k, shape='doublecircle', fontsize=length, style="filled", color="#000000",
                                    fillcolor="#227ef0"))

      elif v.pID in leaves:
        # elif v.nbrChild() == 0:
        if v.sex == 1:
          graph.add_node(pydot.Node(k, shape='doublebox', fontsize=length, style="filled", color="#00ffff",
                                    fillcolor="#C2F732.0"))
        elif v.sex == 2:
          graph.add_node(pydot.Node(k, shape='doublebox', fontsize=length, style="filled", color="#ff009c",
                                    fillcolor="#C2F732"))
        elif v.sex == 3:
          graph.add_node(pydot.Node(k, shape='doublebox', fontsize=length, style="filled", color="white",
                                    fillcolor="#C2F732"))
        else:
          graph.add_node(pydot.Node(k, shape='doublebox', fontsize=length, style="filled", color="#000000",
                                    fillcolor="#C2F732"))
      else:
        if v.sex == 1:
          graph.add_node(pydot.Node(k, shape='diamond', fontsize=length, style="filled", color="#00ffff",
                                    fillcolor="#ffa600"))
        elif v.sex == 2:
          graph.add_node(pydot.Node(k, shape='diamond', fontsize=length, style="filled", color="#ff009c",
                                    fillcolor="#ffa600"))
        elif v.sex == 3:
          graph.add_node(pydot.Node(k, shape='diamond', fontsize=length, style="filled", color="white",
                                    fillcolor="#ffa600"))
        else:
          graph.add_node(pydot.Node(k, shape='diamond', fontsize=length, style="filled", color="#000000",
                                    fillcolor="#ffa600"))
  ...
  def generation_pedigree(self, famID, nbDepart, nbGeneration):
    ...
    """
    nbChildMax = 10  # Nombres d'enfants max que l'on peut avoir
    ...
    for nb in range(nbGeneration):
      if nb == 0:
        ...
        for i in range(1, nbDepart + 1):
          self.add_people(famID, str(i), self.no_people, self.no_people)
          currentID += 1
        ...
        members = self.get_pedigree()  # Inutile pour l'instant
        # On récupère un set des hommes, un autre des femmes afin d'effectuer les premiers mariages
        first_male = self.get_male()
        first_female = self.get_female()

        # Mariage et enfants
        nbMariage = nbDepart // 2  #  Permet d'effectuer le plus de mariage possible
        ...
        while random.random() < pMariage and len(people) > 1:
          Consanguinity = random.random()

          if Consanguinity < pConsanguinity:
            print("people au tout debut", people)
            print("ped au tout debut", self._pedigree)
            mom, dad = random.sample(people, 2)
            # A refaire, on fait de la consanguinité 'direct'
            while self.get_people(mom).matID != self.get_people(dad).matID or self.get_people(
                mom).fatID != self.get_people(dad).fatID:
              mom, dad = random.sample(people, 2)
            # while not self.is_consanguineous(mom, dad, nb):
            #     print('mom',mom in self._pedigree.keys())
            #     print('dad',dad in self._pedigree.keys())
            #     mom, dad = random.sample(people, 2)
            # Choix du nombre d'enfants (entre 1 et nbChildMax)
            ...
            for i in range(child):
              ...
              currentID += 1
            print("keys", sorted(set(self._pedigree.keys())))
            print('len', len(people), "people", sorted(people))
            print(people.difference(set(self._pedigree.keys())))
            print('mom', mom, 'dad', dad)
            people.remove(mom)
            people.remove(dad)
            print('len2', len(people), "people", sorted(people))
            ...
            if random.random() < 0.5:  # Une chance sur deux de créer un nouvel individu pour le mariage
              if len(people) > 2: mom, dad = random.sample(people, 2)
              while self.get_people(mom).matID == self.get_people(dad).matID and self.get_people(
                  mom).fatID == self.get_people(dad).fatID:
                if len(people) > 2: mom, dad = random.sample(people, 2)
                # Choix du nombre d'enfants (entre 1 et nbChildMax)
                child = random.randint(0, nbChildMax)
              ...
              tmp = currentID
              self.add_people(famID, str(tmp), self.no_people, self.no_people)
              people.add(tmp)
              currentID += 1
              # Choix du nombre d'enfants (entre 1 et nbChildMax)
              child = random.randint(0, nbChildMax)
  ...
  def generation_ped2(self, famID, nbDepart, nbGeneration):
    """
    Return a new pedigree, start with a number nbDepart of people and create nbGeneration
    """
    ...
    for n in range(nbGeneration):
      ...
      if n == 0:  # IE generation de depart
        ...
        while random.random() < pMariage:
          if random.random() < 0.5:  # Mariage entre deux personnes existantes du pedigree
            if len(mariage) > 1:
            ...
            # Ajout d'un nouveau membre dans le pedigree
            if len(mariage) > 0:
