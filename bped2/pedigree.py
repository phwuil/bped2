...
class Pedigree:
    ...
    def update_children(self, pID):
        ...
        if father in self._pedigree:
            self._pedigree[father].child.add(pID)
        if mother in self._pedigree:
            self._pedigree[mother].child.add(pID)
    ...
    def remove_family(self, famID):
        ...
        for k, v in list(self._pedigree.items()):
            if v.famID == famID:
                del self._pedigree[
                    k]  # Pas besoin de faire attention aux liens avec les autres puisqu'on supprime toute la famille
            ...
            for n in range(nbGeneration):
                ...
                if n == 0: #IE generation de depart
                    for p in range(nbDepart):
                        self.add_people(famID,str(currentID),'0','0')
                        currentID += 1
